...
int main()
{
	...
	// std::string path = Unicode2Utf8(L"股市讨论.mp4"); // 方式1
	// libvlc_media_t* media = libvlc_media_new_path(vlc_ins, path.c_str());

	std::string path = Unicode2Utf8(L"file:///D:\\1_zhangjiahao\\Code\\VideoPlay\\VideoPlay\\股市讨论.mp4"); // 方式2
	...
	do
	{
		...
		int heigth = libvlc_video_get_height(player);
		printf("width = %d; hegith = %d\r\n", width, heigth);

		while (!_kbhit()) {
			printf("%f%%\r", 100.0 * libvlc_media_player_get_position(player)); // 播放进度
			Sleep(500);
		}
		getchar();
